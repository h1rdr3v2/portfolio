			...
			<div className="flex flex-col gap-4">
				<div className="flex flex-col gap-2.5 w-full">
					...
					{currentRoles.map((role, index) => (
						<Card
							key={index}
							className="flex flex-col gap-y-1 p-3.5 rounded-lg border-0"
						>
							<CardHeader className="flex flex-col-reverse md:flex-row justify-between md:items-center gap-y-1 p-0">
								...
								{role.status && (
									<Badge
										...
										style={{ backgroundColor: "rgba(138,187,76,.2)" }}
									>
										<span
											className="size-2 rounded-[0.1875rem] mr-1"
											style={{ backgroundColor: role.statusColor }}
										></span>
										{role.status}
									</Badge>
								)}
				...
				<div className="flex flex-col gap-y-2.5">
					<div
						...
						onClick={() => setShowFormerRoles(!showFormerRoles)}
					>
						<Badge variant="outline">Former Roles</Badge>
						{showFormerRoles ? arrowUpIcon() : moreInfoIcon()}
					</div>
					<div
						className={`transition-all duration-300 space-y-2.5 overflow-hidden ${showFormerRoles ? "max-h-screen opacity-100" : "max-h-0 opacity-0"}`}
					>
						{formerRoles.map((role, index) => (
							<Card
								key={index}
								className="flex flex-row gap-y-1 p-3.5 rounded-lg border-0 justify-between items-center space-y-0"
							>
