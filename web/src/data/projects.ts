...
/**
 * The three that open the site. Order here is the order you scroll through.
 * Each one needs real screenshots — the phone frame is the whole point of the
 * section, so a project without images does not belong here.
 */
export const featuredProjects: FeaturedProject[] = [
	{ slug: "ledgercopilot" },
	{ slug: "mysales" },
	{ slug: "mycgpa" },
	// `metrics` is intentionally empty on all three. Add real numbers here
	// (e.g. { value: "18,402", label: "statements parsed" }) and the showcase
	// will render the stat row automatically.
]
...
/** Filter order in the UI — "all" first, then by how much work sits behind it. */
export const categoryOrder: (ProjectCategory | "all")[] = [
	"all",
	"mobile",
	"website",
	"bot",
...
/** Featured slugs, for excluding them from the "everything else" grid. */
export const featuredSlugs = new Set(featuredProjects.map((f) => f.slug))
